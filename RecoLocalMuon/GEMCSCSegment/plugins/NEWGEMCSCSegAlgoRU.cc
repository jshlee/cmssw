/**
 * \file NEWGEMCSCSegAlgoRU.cc
 * originally based on CSCSegAlgoRU.cc
 * to use GEMCSCSegFit class
 */

#include "NEWGEMCSCSegAlgoRU.h"
 ...
 *  Constructor
 */
NEWGEMCSCSegAlgoRU::NEWGEMCSCSegAlgoRU(const edm::ParameterSet& ps) : NEWGEMCSCSegmentAlgorithm(ps), myName("NEWGEMCSCSegAlgoRU"), sfit_(nullptr) 
{	 
 ...
 * Destructor
 */
NEWGEMCSCSegAlgoRU::~NEWGEMCSCSegAlgoRU() {}
 ...
 * Run the algorithm
 */
std::vector<GEMCSCSegment> NEWGEMCSCSegAlgoRU::run( const std::map<uint32_t, const CSCLayer*>& csclayermap,
						    ...
						    const std::vector<const GEMRecHit*>& gemrechits) 
{
  ...
  edm::LogVerbatim("NEWGEMCSCSegAlgoRU") << "[NEWGEMCSCSegAlgoRU::run] cached the csclayermap and the gemrollmap";

  // --- LogDebug for CSC Layer map -----------------------------------------
  std::stringstream csclayermapss; csclayermapss<<"[NEWGEMCSCSegAlgoRU::run] :: csclayermap :: elements ["<<std::endl;
  ...
  edm::LogVerbatim("NEWGEMCSCSegAlgoRU") << csclayermapstr;
  // --- End LogDebug -------------------------------------------------------

  // --- LogDebug for GEM Eta Partition map ---------------------------------
  std::stringstream gemetapartmapss; gemetapartmapss<<"[NEWGEMCSCSegAlgoRU::run] :: gemetapartmap :: elements ["<<std::endl;
  ...
  edm::LogVerbatim("NEWGEMCSCSegAlgoRU") << gemetapartmapstr;
  // --- End LogDebug -------------------------------------------------------
  ...
  for(std::vector<const CSCRecHit2D*>::const_iterator cscSegIt = cscsegments.begin(); cscSegIt != cscsegments.end(); ++cscSegIt)
    {
  ...
  edm::LogVerbatim("NEWGEMCSCSegAlgoRU") << "[NEWGEMCSCSegAlgoRU::run] GEMCSC Segments fitted or wrapped, size of vector = "<<segmentvectorfinal.size();
 ...
 */
std::vector<const TrackingRecHit*> NEWGEMCSCSegAlgoRU::chainHitsToSegm(const CSCRecHit2D* cscsegment, const std::vector<const GEMRecHit*>& gemrechits) 
{
 ...
 */
std::vector<GEMCSCSegment> NEWGEMCSCSegAlgoRU::buildSegments(const CSCRecHit2D* cscsegment, const std::vector<const TrackingRecHit*>& rechits) 
{
  ...
  edm::LogVerbatim("NEWGEMCSCSegAlgoRU") << "[NEWGEMCSCSegAlgoRU::buildSegments] fit done, will now try to make GEMCSC Segment from CSC Segment in "<<cscsegment->cscDetId()
  ...
  edm::LogVerbatim("NEWGEMCSCSegAlgoRU") << "[NEWGEMCSCSegAlgoRU::buildSegments] GEMCSC Segment made";
