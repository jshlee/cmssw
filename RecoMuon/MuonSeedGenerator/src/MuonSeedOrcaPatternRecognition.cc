                                             ...
                                             std::vector<MuonRecHitContainer> & result)
{
  ...
  for ( iter = cscForwardLayers.begin(); iter != cscForwardLayers.end(); iter++ )
    std::cout << (*iter)->location() << " " << (*iter)->subDetector()
	      << " z "  << (*iter)->surface().position().z()
	      <<endl;

  
  // // get the GEM layers
  vector<DetLayer*> gemForwardLayers;
  vector<DetLayer*> gemBackwardLayers;
  const DetLayer* GE0Bwd = 0;
  const DetLayer* GE0Fwd = 0;
  const DetLayer* GE1Bwd = 0;
  const DetLayer* GE1Fwd = 0;
  const DetLayer* GE2Bwd = 0;
  const DetLayer* GE2Fwd = 0;
  const DetLayer* GE3Bwd = 0;
  const DetLayer* GE3Fwd = 0;
  const DetLayer* GE4Bwd = 0;
  const DetLayer* GE4Fwd = 0;
  const DetLayer* GE5Bwd = 0;
  const DetLayer* GE5Fwd = 0;
  
  if (enableGEMMeasurement){
    gemForwardLayers  = muonLayers->forwardGEMLayers();
    gemBackwardLayers = muonLayers->backwardGEMLayers();

    // temp due to different gem layers
    if (gemBackwardLayers.size()){
      GE0Bwd = gemBackwardLayers[0];
      GE0Fwd = gemForwardLayers[0];
      GE1Bwd = gemBackwardLayers[1];
      GE1Fwd = gemForwardLayers[1];
      if (gemBackwardLayers.size() > 2){
	GE2Bwd = gemBackwardLayers[2];
	GE2Fwd = gemForwardLayers[2];
	GE3Bwd = gemBackwardLayers[3];
	GE3Fwd = gemForwardLayers[3];
	GE4Bwd = gemBackwardLayers[4];
	GE4Fwd = gemForwardLayers[4];
	GE5Bwd = gemBackwardLayers[5];
	GE5Fwd = gemForwardLayers[5];
      }
    }
  }
  
  std::cout << "gemForwardLayers.size() "<< gemForwardLayers.size() <<endl;
  std::cout << "cscForwardLayers.size() "<< cscForwardLayers.size() <<endl;
  ...
  MuonDetLayerMeasurements muonMeasurements(theDTRecSegmentLabel.label(),theCSCRecSegmentLabel,edm::InputTag(),theGEMRecSegmentLabel,enableDTMeasurement,enableCSCMeasurement,false,enableGEMMeasurement);
  ...
  MuonRecHitContainer ge11fwdRecHits = filterSegments(muonMeasurements.recHits(GE0Fwd,event), endcapdThetaCut);
  //  MuonRecHitContainer gemtmp = filterSegments(muonMeasurements.recHits(GE1Fwd,event), endcapdThetaCut);
  //copy(gemtmp.begin(),gemtmp.end(),back_inserter(ge11fwdRecHits));
  cout << "ge11fwdRecHits.size() "<< ge11fwdRecHits.size() << endl;
  MuonRecHitContainer ge11bwdRecHits = filterSegments(muonMeasurements.recHits(GE0Bwd,event), endcapdThetaCut);
  //gemtmp = filterSegments(muonMeasurements.recHits(GE1Bwd,event), endcapdThetaCut);
  //copy(gemtmp.begin(),gemtmp.end(),back_inserter(ge11bwdRecHits));
  cout << "ge11bwdRecHits.size() "<< ge11bwdRecHits.size() << endl;

  MuonRecHitContainer me11fwdRecHits = filterSegments(muonMeasurements.recHits(ME11Fwd,event), endcapdThetaCut);
  //  MuonRecHitContainer gemtmp = filterSegments(muonMeasurements.recHits(GE1Fwd,event), endcapdThetaCut);
  //copy(gemtmp.begin(),gemtmp.end(),back_inserter(ge11fwdRecHits));
  cout << "me11fwdRecHits.size() "<< me11fwdRecHits.size() << endl;
  MuonRecHitContainer me11bwdRecHits = filterSegments(muonMeasurements.recHits(ME11Bwd,event), endcapdThetaCut);
  //gemtmp = filterSegments(muonMeasurements.recHits(GE1Bwd,event), endcapdThetaCut);
  //copy(gemtmp.begin(),gemtmp.end(),back_inserter(ge11bwdRecHits));
  cout << "me11bwdRecHits.size() "<< me11bwdRecHits.size() << endl;

  endcapPatterns(ge11bwdRecHits,
  ...
  if(result.empty()) 
    {
      ...
      if (enableGEMMeasurement){
	if (gemBackwardLayers.size() > 2){
	  tmp = filterSegments(muonMeasurements.recHits(GE5Bwd,event), endcapdThetaCut);
	  copy(tmp.begin(),tmp.end(),back_inserter(all));
	  tmp = filterSegments(muonMeasurements.recHits(GE4Bwd,event), endcapdThetaCut);
	  copy(tmp.begin(),tmp.end(),back_inserter(all));
	  tmp = filterSegments(muonMeasurements.recHits(GE3Bwd,event), endcapdThetaCut);
	  copy(tmp.begin(),tmp.end(),back_inserter(all));
	  tmp = filterSegments(muonMeasurements.recHits(GE2Bwd,event), endcapdThetaCut);
	  copy(tmp.begin(),tmp.end(),back_inserter(all));
	}      
	tmp = filterSegments(muonMeasurements.recHits(GE1Bwd,event), endcapdThetaCut);
	copy(tmp.begin(),tmp.end(),back_inserter(all));
	...
	copy(tmp.begin(),tmp.end(),back_inserter(all));
	tmp = filterSegments(muonMeasurements.recHits(GE1Fwd,event), endcapdThetaCut);
	copy(tmp.begin(),tmp.end(),back_inserter(all));
      	if (gemBackwardLayers.size() > 2){

	  tmp = filterSegments(muonMeasurements.recHits(GE2Fwd,event), endcapdThetaCut);
	  copy(tmp.begin(),tmp.end(),back_inserter(all));
	  tmp = filterSegments(muonMeasurements.recHits(GE3Fwd,event), endcapdThetaCut);
	  copy(tmp.begin(),tmp.end(),back_inserter(all));
	  tmp = filterSegments(muonMeasurements.recHits(GE4Fwd,event), endcapdThetaCut);
	  copy(tmp.begin(),tmp.end(),back_inserter(all));
	  tmp = filterSegments(muonMeasurements.recHits(GE5Fwd,event), endcapdThetaCut);
	  copy(tmp.begin(),tmp.end(),back_inserter(all));
	}
      }
      // end gem
						    ...
						    std::vector<MuonRecHitContainer> & result)
{
  ...
  //bool* GE1 = zero(ge11.size());
  // bool* ME1 = zero(list1.size());
  // MuonRecHitContainer listgem = ge11;

  // // creates list of compatible track segments
  // //  if (list1.size() == 0) list1 = ge11;

  // for (MuonRecHitContainer::iterator iter = listgem.begin(); iter!=listgem.end(); iter++ ){
  //   MuonRecHitContainer seedSegments;
  //   seedSegments.push_back(*iter);
  //   complete(seedSegments, list1, ME1);
  ...
  for (MuonRecHitContainer::iterator iter = list1.begin(); iter!=list1.end(); iter++ ){
    ...
    MuonRecHitContainer seedSegments;
    seedSegments.push_back(*iter);
    //complete(seedSegments, ge11, GE1);
    complete(seedSegments, list2, ME2);
    complete(seedSegments, list3, ME3);
    complete(seedSegments, list4, ME4);
    complete(seedSegments, list5, ME5);
...
MuonSeedOrcaPatternRecognition::filterSegments(const MuonRecHitContainer & segments, double dThetaCut) const
{
  ...
  for(MuonRecHitContainer::const_iterator segmentItr = segments.begin();
      ...
      if((*segmentItr)->isDT())
	{
      ...
      else if((*segmentItr)->isCSC() || (*segmentItr)->isGEM()) 
	{
...
{
  if (segment->isCSC())
    return (CSCDetId(segment->geographicalId()).ring() == 4);
  else
    return false;
